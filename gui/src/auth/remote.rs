...
//! Remote SLA signer, ported from `antumbra` (TUI). Talks to a Penumbra
//! signing server to authorize and sign SLA / DAA challenges on devices that
//! cannot be bypassed by an exploit.
...
impl RemoteSigner {
    pub const fn new(config: Arc<PenumbraGuiConfig>) -> Self {
        Self { config }
    }

    fn get_config_path() -> Option<PathBuf> {
        let dir = dirs_next::config_dir()?.join("penumbra-gui");
        if !dir.exists() {
